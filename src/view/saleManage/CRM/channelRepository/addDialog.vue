  ...
  <div class="com-dialog-container" v-loading="dataLoading">
    <div class="com-dialog">
				...
				<el-row class="el-row-cla">
					<el-col :span="24">
						<el-form-item label='客户类型'>
							<el-radio v-model='visitorType' :label="1">访客</el-radio>
							<el-radio v-model='visitorType' :label="2">准客户</el-radio>
						</el-form-item>
					</el-col>
				</el-row>
        
        <div v-if='visitorType == 1'>
          <el-row class='el-row-cla'>
            <el-col :span="8">
              <el-form-item label='访客名称'>
                <el-input type="text" placeholder='请填写访客名称' v-model="visForm.name"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label='访客联系人'>
                <el-input type="text" v-model="visForm.contactName" placeholder='请填写客户联系人'></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label='联系电话'>
                <el-input type="text" v-model="visForm.phone" placeholder="请填写客户联系电话"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row class='el-row-cla'>
            <el-col :span="8">
              <el-form-item label='联系人职务'>
                <el-input type="text" v-model="visForm.contactPosition" placeholder="请填写联系人职务"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label='访客地区'>
                <AreaSelect ref="visAreaSe"
                            placeholder='请选择访客地区'
                            :area="(visForm.provinceName?visForm.provinceName:'') + ' ' + (visForm.cityName?visForm.cityName:'')  + ' ' + (visForm.areaName?visForm.areaName:'')"
                            @change="areaSelectedOptionsHandleChangeVis"
                            :selectLastLevelMode="true"></AreaSelect>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label='访客行业'>
                <el-cascader
                  style="width: 100%"
          ...
          <el-row class='el-row-cla'>
            <el-col :span="8">
              <el-form-item label='访客来源'>
                <el-cascader
                  placeholder='请选择访客来源'
                  :options="visitorResourceType"
                  :change-on-select="true"
                  v-model="visitorResource"
                  @active-item-change="chanceSourceChangeHandles"
                  @change="chanceSourceChangeHandles"
                  :props="props"
            ...
            <el-col :span="8">
              <el-form-item label='访客推荐人' prop="provider">
                <el-select v-model.number="visForm.visitorReferrer" filterable placeholder="请选择访客推荐人">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button class="save-button" @click="saveSubmitForm('addForm')" v-if='visitorType == 2'>确 定</el-button>
        <el-button class="save-button" @click="saveVisSubmitForm('addForm')" v-if='visitorType == 1'>确 定</el-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    visForm: { //访客表单
      ...
      cityName: '',
      areaName: '',
      visitorResource: '',             //访客来源
      // visitorResourceName: '',         //访客来源name
			visitorReferrer: '',             // 访客推荐人
    },
    addForm: { // 添加表单
    ...
    methods: {
		...
		areaSelectedOptionsHandleChangeVis (value) {
			let name = this.$refs.visAreaSe.getSelectedName(value)
			this.visForm.provinceId = value[0] || ''
			this.visForm.cityId = value[1] || ''
			this.visForm.areaId = value[2] || ''
			this.visForm.provinceName = name[0] || ''
			this.visForm.cityName = name[1] || ''
			this.visForm.areaName = name[2] || ''
		},
		// 获取所有需求提供人
		getAllStaff () {
    ...
    saveVisSubmitForm (formName) {
      ...
      this.visForm.industry = this.addForm.industryArr.join(',')
      if(this.params.detail) { //编辑
        
      }else {
        this.visForm.visitorResourceName = this.traverseTreeVis(this.visForm.visitorResource)
        // console.log(this.visForm);
        // return;
        API.salesOpportunities.addVisitor(this.visForm, (data) => {
          if (data.status) {
    ...
    saveSubmitForm (formName) {
        ...
        API.common.codeConfig({type: type, pCode: pCode}, (data) => {
          if (type === 2) {
            ...
            if (this.visitorResourceType.length === 0) {
              ...
              this.visitorResource.push(this.params.topSource[2].value);
              this.chanceSourceChangeHandles([this.params.topSource[2].value]); // 默认获取第二级
      ...
      chanceSourceChangeHandles (va) {
        this.getLastItem(this.visitorResourceType, va, 'id')
        API.common.codeConfig({type: 5, pCode: va[va.length - 1]}, (data) => {
          // console.log('目标item:', this.targetObj)
          if (data.data.length) {
            let arr = data.data.map((item) => {
        ...
        this.visForm.visitorResource = va.join('-')
      },
      ...
      treeGetNameVis (id, node) { // 遍历树获取名称
        if (!node) {
          return ''
        }
        if (node && node.length > 0) {
          ...
          for (i = 0; i < node.length; i++) {
            if (id === node[i].id) {
              this.visNameArr.push(node[i].codeName)
              return node[i].codeName
            } else {
              this.treeGetNameVis(id, node[i].children)
            }
          }
        }
      },
      ...
      traverseTreeVis (source, node) { // 遍历树
        if (!source) {
          return
        }
        let sourceArr = source.split('-')
        this.visNameArr = [] // 初始
        sourceArr.forEach((item, index) => {
          this.treeGetNameVis(parseInt(item, 10), this.chanceSourceType)
        })
        return this.visNameArr.join('-')
      },
    },
    created () {
      ...
      if (this.params.detail) { // 编辑
        console.log(1111, this.params.detail)
        this.detail = this.params.detail;
        this.visitorType = this.detail.visitorType;
        if(this.detail.visitorType === 1) {
