...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveSubmitForm (formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            // 先调接口查看是否有权限
            API.salesOpportunities.getCustomerRenewalRoel((data) => {
              if(data.data) {
                ...
                this.$message.warning('您没有客户续签的权限。');
              }
            })
          } else {
    ...
    created () {
      ...
      this.addForm.salerName = webStorage.getItem('userInfo').name;
      this.addForm.salerId = webStorage.getItem('userInfo').id;
    },
  }
</script>
