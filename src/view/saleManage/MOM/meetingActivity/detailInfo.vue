      ...
      <div class="com-info-left">
        ...
        <div class="com-info-text">
          <h3>{{detailInfo.meetingName}}
            <span
              style="position:relative;top: -5px;font-size: 12px; background: #FF7700; color: #fff;padding: 4px 12px;border:1px solid #FF7700;margin-left: 20px;border-radius: 20px;" v-if='detailInfo.state == 1'>
              {{detailInfo.stateName}}
            </span>
            <span
              style="position:relative;top: -5px;font-size: 12px; background: #26C6DA; color: #fff;padding: 4px 12px;border:1px solid #26C6DA;margin-left: 20px;border-radius: 20px;" v-if='detailInfo.state == 2'>
              {{detailInfo.stateName}}
            </span>
            <span
              style="position:relative;top: -5px;font-size: 12px; background: #C4C4C4; color: #fff;padding: 4px 12px;border:1px solid #C4C4C4;margin-left: 20px;border-radius: 20px;" v-if='detailInfo.state == 3'>
              {{detailInfo.stateName}}
            </span>
          </h3>
          <p>
<!--            <span class="com-d-item">负责部门: <span>{{detailInfo.meetingCreatorDepartmentName}}</span></span>
            ...
            <span class="com-d-item">创建人: <span>{{detailInfo.meetingCreatorName}}</span></span>
            <span class="com-d-item">创建日期: <span>{{detailInfo.created && $moment(detailInfo.created).format('YYYY-MM-DD HH:mm:ss')}}</span></span>
            <span class="com-d-item">活动地址: <span>{{detailInfo.address}}</span></span>
          </p>
        </div>
      </div>
					...
					<div class='com-dialog'>
						<table class='com-dialog-table table-line-color'>
							<tr>
								...
								<td>{{detailInfo.applyTimeStart ? $moment(detailInfo.applyTimeStart).format('YYYY/MM/DD') + ' - ' + $moment(detailInfo.applyTimeEnd).format('YYYY/MM/DD') : ''}}</td>
								<td class='td-title'>活动时间</td>
								<td>{{detailInfo.meetingTimeStart ? $moment(detailInfo.meetingTimeStart).format('YYYY/MM/DD HH:mm') + ' - ' + $moment(detailInfo.meetingTimeEnd).format('YYYY/MM/DD HH:mm') : ''}}</td>
							</tr>
							<tr>
								<td class='td-title'>主办单位</td>
								<td colspan="5"><el-tag type="success" style='margin-right: 5px; margin-bottom: 5px; margin-top: 5px; cursor:pointer;' v-for='item in host'><span @click='showErWeiMa(item)'><img style='float: left; width: 15px;height: 15px;margin-top:3px;margin-right: 3px;' src="../../../../assets/icon/erweima.png">{{item.name}}</span></el-tag></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><a :href="detailInfo.meetingActivityPlan" class='acolor'>{{detailInfo.meetingActivityPlanName}}</a></td>
								<td class='td-title'>活动课件</td>
								<td><a :href="detailInfo.meetingCourseware" class='acolor'>{{detailInfo.meetingCoursewareName}}</a></td>
							</tr>
							<tr>
			...
			<div class='meeting-detail-all-right'>
				...
				<div class='all-right-meeting'>
					<div class='right-meeting-meeting'>
						...
						<p class='right-meeting-meeting-lookmore'><span style='cursor: pointer;' @click='lookMoreUpdate'>{{lookMore ? '收起更多' : '查看更多'}}</span></p>
					</div>
				</div>
			</div>
		</div>
...
<!-- 				<el-table-column
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  title="报名二维码"
		  :visible.sync="dialogVisiblesErWeiMa"
		  width="250px">
			<div ref='meetingErWeiMa'>
				<vue-qr
					:text="meetConfig.value"
					:size="200"
					:margin="0"
					:callback="agentRecCallback"
					qid="activity-1"></vue-qr>
					...
					<p>
						<el-button class='erweiBtn' type="primary" v-if="codeImgBase64" @click="downloadCodeHandle3">下载二维码</el-button>
					</p>
			</div>
		</el-dialog>
  </div>
...
<script>
  ...
  import { activityCodePre, activityCodePres} from '../../../../utils/const'
  import Qs from 'qs'
            ...
            this.config1.value = activityCodePre + Qs.stringify({ // 拼装活动二维码参数
              ...
              organizationId: this.detailInfo.meetingCreatorOgrId
						...
						if(this.detailInfo.meetingHeadModel) {
							this.allPersonnel.push({type: '负责人', name: this.detailInfo.meetingHeadModel.name, qqCur: false, wxCur: false, phone: this.detailInfo.meetingHeadModel.mobilePhone, wx: this.detailInfo.meetingHeadModel.wx, qq: this.detailInfo.meetingHeadModel.qq})
						}
						if(this.detailInfo.meetingLecturerModel) {
							this.allPersonnel.push({type: '讲师', name: this.detailInfo.meetingLecturerModel.lectureName, qqCur: false, wxCur: false, phone: this.detailInfo.meetingLecturerModel.lecturePhone, wx: this.detailInfo.meetingLecturerModel.wx, qq: this.detailInfo.meetingLecturerModel.qq})
						}
						if(this.detailInfo.meetingCommissioners.length > 0) {
							this.detailInfo.meetingCommissioners.forEach(a => {
								this.allPersonnel.push({type: '会务专员', name: a.name, phone: a.mobilePhone, qqCur: false, wxCur: false, wx: a.wx, qq: a.qq})
							})
						}
      ...
      sortChangeHandle (sortObj) {
        ...
        if(this.vuewS) {
			...
			getApplyUser () {
				...
				this.getQueryParams();
				API.activity.applyUserZX(Object.assign({}, this.defaultListParams, this.sortObj), (data) => {
			...
			getDemandList () {
				this.dataLoadings = true;
				this.getQueryParams();
				API.salesOpportunities.list(Object.assign({}, this.defaultListParams, this.sortObj), (data) => {
					this.dataLoadings = false;
					if(data.status) {
			...
			showErWeiMa (item) {
				...
				if(!item.agent) {
					this.meetConfig.value = activityCodePre + Qs.stringify({ // 拼装活动二维码参数
						meetingId: this.detailInfo.id,
						channelId: item.id
					})
				}else {
					console.log(activityCodePres)
					this.meetConfig.value = activityCodePres + Qs.stringify({ // 拼装活动二维码参数
						meetingId: this.detailInfo.id,
						channelId: item.id
					})
				}
...
<style scoped lang="scss" rel="stylesheet/scss">
	...
	.all-right-title-meeting, .all-right-title-load {
		display: inline-block;
		float: left;
		height: 36px;
		padding: 0 5px;
