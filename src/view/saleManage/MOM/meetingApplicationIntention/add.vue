    ...
    <div class="com-dialog">
      <el-form :model="form" ref="form" :rules="rules" :disabled="isFormDisabled">
        <table class="com-dialog-table">
          <tr>
            <td class="td-title">请输入意向名称</td>
            <td class="td-text">
              <el-form-item prop="name">
                <el-input type="text" v-model="form.name" :placeholder="'请输入意向名称'" :maxlength="30"></el-input>
              </el-form-item>
            </td>
          </tr>
          <tr v-if="params.action === 'add'">
            <td class="td-title">上级意向</td>
            <td class="td-text">
              <el-form-item>
          ...
          if (valid) {
            switch (that.params.action) {
              case 'add':
                ...
                API.activity.addapplyIntentionListZX(that.form, function (resData) {
                  that.loading = false
                  if (resData.status) {
                    Message({
                      message: '新增成功！',
              ...
              case 'update':
                ...
                API.activity.editapplyIntentionListZX(that.form, function (resData) {
                  that.loading = false
                  if (resData.status) {
                    Message({
                      message: '修改成功！',
