...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$nextTick(() => {
              this.handleReqData()
              API.salesOpportunitiesSea.edit({body: that.ruleForm, path: that.ruleForm.id}, function (resData) {
      ...
      getPollList (id) {
        API.salesOpportunitiesSea.list(id, (da) => {
      ...
      getPoolDetail (id) {
        ...
        API.salesOpportunitiesSea.detail(id, (resD) => {
      ...
      deletePool () {
        this.$confirm('确定删除, 是否继续?', '提示', {
          ...
          API.salesOpportunitiesSea.deleteSea(this.activeIndex, (da) => {
            if (da.status) {
              this.$message.success('操作成功')
              API.salesOpportunitiesSea.list(this.organizationId, (da) => {
                this.pollList = da.data.content
                this.getPoolDetail(this.pollList[0].id)
              })
            }
