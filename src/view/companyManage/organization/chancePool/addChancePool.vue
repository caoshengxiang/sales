        ...
        that.$refs[formName].validate((valid) => {
          if (valid) {
            switch (that.params.type) {
              case 'add':
                that.loading = true
                API.salesOpportunitiesSea.add(Object.assign({}, that.form, {organizationId: that.params.organizationId}),
                  (resData) => {
                    that.loading = false
                    if (resData.status) {
                      Message({
              ...
              case 'edit':
                that.loading = true
                API.salesOpportunitiesSea.edit({body: that.form, path: that.params.id}, function (resData) {
                  that.loading = false
                  if (resData.status) {
                    Message({
                      message: '修改成功！',
